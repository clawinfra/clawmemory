...
export interface RecallContext {
  ...
  contextBlock: string; // formatted for injection into system prompt
}

/**
 * RecallHook retrieves relevant memories before each turn.
 */
export class RecallHook {
   ...
   */
  async onPreTurn(query: string): Promise<RecallContext> {
    ...
    try {
      const result = await this.client.recall(
        query,
        this.config.recall_limit,
        '', // no container filter
        this.config.recall_threshold,
   ...
   */
  private formatContextBlock(facts: RecallResult[], profileSummary: string): string {
    const parts: string[] = [];
