...
export class ClawMemoryTools {
   ...
   */
  async remember(args: string): Promise<ToolResult> {
    ...
    try {
      const result = await this.client.remember(
        ...
        parsed.container ?? 'general',
        parsed.importance ?? 0.7
      );
   ...
   */
  async recall(args: string): Promise<ToolResult> {
    ...
    try {
      ...
      const lines = result.results.map(
        (r, i) => `${i + 1}. [${r.category}] ${r.content} (score: ${r.score.toFixed(3)})`
      );
   ...
   */
  async profile(): Promise<ToolResult> {
    try {
   ...
   */
  async forget(args: string): Promise<ToolResult> {
   ...
   */
  async memoryStats(): Promise<ToolResult> {
    try {
  ...
  /** Parse /remember args. */
  private parseRememberArgs(args: string): {
    content: string;
    category?: string;
    container?: string;
    importance?: number;
    ...
    const parts = args.trim().split(/\s+/);
    const result: { content: string; category?: string; container?: string; importance?: number } = {
      content: '',
    };
  ...
  /** Parse /recall args. */
  private parseRecallArgs(args: string): {
    query: string;
    limit?: number;
    container?: string;
  } {
    const parts = args.trim().split(/\s+/);
    const result: { query: string; limit?: number; container?: string } = { query: '' };
